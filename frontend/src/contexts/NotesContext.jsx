...
const exportSingleNote = async (noteId, title) => {
  try {
    ...
    const res = await fetch(`http://localhost:5000/api/notes/export/${noteId}`, {
      method: "GET",
      headers: {
        Authorization: `Bearer ${token}`,
      },
...
return (
    ...
    value={{notes, loading, error, fetchNotes, addNote,updateNote, deleteNote, searchNotes, searchQuery, setSearchQuery, togglePin,exportNotes, importNotes, exportSingleNote
}}
    >{children}
    </NotesContext.Provider>
);
